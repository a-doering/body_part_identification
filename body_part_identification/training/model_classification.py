...
def build_model(patch_size, n_classes):
    '''
    Model is build after single stage model of Zhang
    ...
    landmark_class_probability = Dense(n_classes, activation='softmax')(dense_3)
    
    # Wrap in a Model
    model = Model(inputs=input, outputs=landmark_class_probability)
    return model
